...
module.exports = async (client, message) => {
    ...
    await pool.query(`SELECT * FROM person WHERE user_id = $1`, [message.author.id], async (err, result) => {
        if (message.author.id.includes(result.rows)) {
            await pool.query("INSERT INTO person (user_id) VALUES ($1) RETURNING *", [message.author.id])
        }
    })

    await pool.query(`SELECT * FROM guild WHERE guild_id = $1`, [message.guildId], async (err, result) => {
        if (message.guildId.includes(result.rows)) {
            await pool.query("INSERT INTO guild (guild_id) VALUES ($1) RETURNING *", [message.guildId])
        }
    })
