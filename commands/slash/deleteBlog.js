...
Если вы удалите то потеряете все сообщения!`, components: [button]});
    ...
    collector.on("end", async (ButtonInteraction) => {
        ...
        if(id === "delete") {
            ...
            await pool.query("UPDATE person SET channel_id = 'None', blog = false WHERE user_id = $1", [interaction.user.id]);
            return ButtonInteraction.first().followUp({content: "Блог был удален!"});
        }
        if(id === "cancel") return ButtonInteraction.first().followUp({content: "Блог не будет удален!"});
